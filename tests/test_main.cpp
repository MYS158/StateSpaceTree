...
static void test_node_child() {
    ...
    CHECK(child->depth() == 1);
    CHECK(child->cost().has_value());
    CHECK(*child->cost() == 1.5);
}
...
static void test_tree_expand() {
    ...
    CHECK(children[0]->state() == 1);
    CHECK(children[1]->state() == 2);
    CHECK(children[2]->state() == 3);
...
static void test_tree_path_to() {
    ...
    CHECK(path.size() == 3);
    CHECK(path[0]->state() == 0);
    CHECK(path[1]->state() == 1);
    CHECK(path[2]->state() == 2);
}
...
static void test_tree_cost_to_missing_edge() {
    ...
    CHECK(!tree.cost_to(lvl2[0]).has_value());
}
...
static void test_strategy_stubs_return_nullopt() {
    ...
    CHECK(!BFS<int>{}.solve(bfs_tree, prob).has_value());
    CHECK(!DFS<int>{}.solve(dfs_tree, prob).has_value());
    CHECK(!Backtracking<int>{}.solve(bt_tree, prob).has_value());
    CHECK(!UCS<int>{}.solve(ucs_tree, prob).has_value());
}

int main() {
    RUN_TEST(test_node_root);
    RUN_TEST(test_node_child);
    RUN_TEST(test_node_depth_chain);
    RUN_TEST(test_node_multiple_children);
    RUN_TEST(test_node_string_state);

    RUN_TEST(test_tree_root);
    RUN_TEST(test_tree_expand);
    RUN_TEST(test_tree_path_to);
    RUN_TEST(test_tree_path_to_root);
    RUN_TEST(test_tree_cost_to_weighted);
    RUN_TEST(test_tree_cost_to_missing_edge);
    RUN_TEST(test_tree_cost_to_root);

    RUN_TEST(test_strategy_interface_extensible);
    RUN_TEST(test_strategy_stubs_return_nullopt);
